...
- **Existing videos:** Open “제목·썸네일 보기” (titles and thumbnail) and select “추천 제목·썸네일 생성” to prepare these assets without recreating the video. Failures preserve the finished video and allow asset retries.
- **Thumbnail publishing:** After uploading a video to YouTube, the adapter attempts to apply its cover through `thumbnails.set`. Cover failures appear separately and never trigger duplicate video uploads. TikTok and Instagram currently offer the image as a download only. Actual thumbnail support depends on platform and channel permissions.
